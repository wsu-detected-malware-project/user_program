...
namespace user_program.Controller{
    public class FormController{
        ...
        public void UpdateForm1LabelsFromCsv(string language)
        {
            string filePath = Env.GetString("LANGUAGE", Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "language.csv"));
            //string selectedLanguage = Environment.GetEnvironmentVariable("COMBOBOX_LANGUAGE") ?? "Korean";
            ...
            using (var reader = new StreamReader(filePath, Encoding.UTF8)) 
            {
                ...
                while (!reader.EndOfStream)
                {
                    ...
                    if (parts.Length >= 12 && parts[0].Trim() == language)
                    {
                        ...
                        Real_Time_OnOffSwitch_Set(parts[6].Trim()); // 실시간 검사 on/off        
                        Real_Time_Cycle_Set(parts[7].Trim()); // 실시간 검사 주기 설정
                        WhiteList_Data_Set(parts[8].Trim()); // 검색 제외 항목                    
                        Update_Label_Set(parts[9].Trim()); // 업데이트
                        Update_Check_Set(parts[10].Trim()); // 업데이트 확인                        
                        Language_Set(parts[11].Trim()); // 언어
